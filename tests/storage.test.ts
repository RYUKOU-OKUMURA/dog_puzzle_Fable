...
describe('normalizeSaveData: おとフィールド(M13)', () => {
  it('soundEnabled: true を通す', () => {
    expect(normalizeSaveData({ version: 2, soundEnabled: true }).soundEnabled).toBe(true);
  });

  it('soundEnabled: false を通す', () => {
    expect(normalizeSaveData({ version: 2, soundEnabled: false }).soundEnabled).toBe(false);
  });

  it('フィールド無しの旧セーブは ON(true)へ補完', () => {
    expect(normalizeSaveData({ version: 2, zukan: {}, stages: {} }).soundEnabled).toBe(true);
  });

  it('型違い(文字列・数値・null)は ON にフォールバック', () => {
    expect(normalizeSaveData({ soundEnabled: 'false' }).soundEnabled).toBe(true);
    expect(normalizeSaveData({ soundEnabled: 0 }).soundEnabled).toBe(true);
    expect(normalizeSaveData({ soundEnabled: null }).soundEnabled).toBe(true);
  });
});
